...
namespace Tmpl8
{
    ...
    "kcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXcbXfb-fb-fb-fb-fb-fb-fb-fb-fb-fb-fb-cbXfe-ec-fe-fe-fd-fb-fb-fb-cbXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcX",
    "kcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXcbXfb-fb-fb-fb-fb-fb-fb-fb-fb-fb-fb-cbXfb-fa-fb-fb-fb-fb-fb-fb-cbXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcXkcX",
    ...
    public:
        ...
        Tank(char loadMap[MapHeight][154])
        {
        ...
        Tank(char loadMap[MapHeight][154], int startx, int starty)
        {
            px = startx;
            py = starty;
            for (int y = 0; y < MapHeight; y++)
        ...
        bool Tank::CheckTankCollisionFixed(int x, int y)
        {
            int left = x + collisionOffsetX;
            int top = y + collisionOffsetY;
            int right = left + collisionWidth;
            ...
            // verificăm colțurile
            if (map[ty1][tx1 * 3 + 2] == 'X') return false;
            if (map[ty1][tx2 * 3 + 2] == 'X') return false;
            if (map[ty2][tx1 * 3 + 2] == 'X') return false;
            if (map[ty2][tx2 * 3 + 2] == 'X') return false;
    ...
    Tank mytank(map2);
    Tank mytank2(map2, 100, 300);
    Map mymap(map2);

    Tank MyTanks[5] = {
        Tank(map2, 80 + IRand(665 - 80), IRand(480)),
        Tank(map2, 80 + IRand(665 - 80), IRand(480)),
        Tank(map2, 80 + IRand(665 - 80), IRand(480)),
        Tank(map2, 80 + IRand(665 - 80), IRand(480)),
        Tank(map2, 80 + IRand(665 - 80), IRand(480))
    ...
    void Game::Tick(float deltaTime)
    {
        ...
        //mytank.Move(screen);
        //mytank2.MoveFixedTankRight(screen);
        for(int i=0;i<=4;i++)
